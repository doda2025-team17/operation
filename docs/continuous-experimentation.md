...
This is chosen intentionally to create cache opportunities in v2, while v1 always forwards the requests to the model-service.

## Accessing Stable and Canary Versions

To enable deterministic testing of both versions, separate hostnames are configured through Istio and exposed via Helm values:
